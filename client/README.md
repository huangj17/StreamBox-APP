...
1. **添加配置源** — 设置 → 配置源管理，粘贴 CMS 接口、TVBox 单仓/多仓或 OuonnkiTV JSON 列表 URL。集合展开后可逐个启用片源；相同接口自动去重，异常源集中收纳并可重新检测。
2. **（可选）启用 JAR 源** — 启动 [JAR Bridge](../jar-bridge/) 服务后，添加源 `http://localhost:9978`
3. **浏览内容** — 在片源行点击「设为首页源」，或通过首页顶部切换。首页只加载所选片源的分类、Banner 与视频列表。
4. **搜索影片** — 汇总所有已启用且未检测失败的片源，结果随返回随展示，卡片标明来源；切换首页源不改变搜索范围。
5. **观看影片** — 视频卡片 → 详情页 → 选集播放，页面显示片源和播放线路，自动记录进度。
